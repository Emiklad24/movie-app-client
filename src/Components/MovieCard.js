...
class MovieCard extends Component {
    ...
    deleteWatchlist = async (movie) => {
        ...
        if (!isAuthenticated) {
            ...
            swal({
                title: `Delete ${movieName} ?`, text: `You are about to remove ${movieName} from your watchlist`, buttons: ["Cancel", ` Delete ${movieName} `], dangerMode: true,
            })
                .then((willRemove) => {
                    if (willRemove) {
                        deleteMovie();
    ...
    viewMore = (movie) => {
        ...
        const movieName = movie.title || movie.original_name || movie.original_title;
        swal({
            title: `${movieName}`, text: `${movie.overview}`, buttons: ["OK", "View more"], dangerMode: true
        })
            .then((willView) => {
                if (willView) {
    ...
    addMovieToWatchList = async (movie) => {
        ...
        if (!isAuthenticated) {
            swal({ title: ` Sorry 😔 😔 😔`, text: `You have to log in to add "${movieWatchlist}" to your watchlist`, buttons: ["OK", "Login"], dangerMode: true })
                .then((willLogin) => {
                    if (willLogin) {
        ...
        try {
            ...
            const addedWatchlistName = addedWatchlist.title || addedWatchlist.original_name || addedWatchlist.original_title || "Movie";
                    ...
                    <div className="card-content">
                        ...
                        <div className="movie-content">
                            <div className="movie-action-buttons">
                                ...
                                {
                                    movie.overview ?
                                        <div className="watchlist-btn peep-view" title={`View more Info about "${currentMovieName}"`} onClick={() => this.viewMore(movie)}>
...

                                {
                                    onWatchList !== false && isLoading === false ?
                                        <>
                                            <div className="watchlist-btn" onClick={() => this.addMovieToWatchList(movie)} title={`Add "${currentMovieName}" to your watchlist`}>
                            ...
                            <Link className="movie-title" to={urlPath}
                                onClick={this.someMethod}>
                            ...
                            {movie.release_date ?
                                ...
                                movie.first_air_date ?
                                    <p>First Air Date: {movie.first_air_date}</p> : null
                            }
