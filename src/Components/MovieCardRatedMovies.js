...
class MovieCard extends Component {
    ...
    promptDelete = async (movie) => {
        ...
        if (!isAuthenticated) {
            ...
            swal({
                title: `Delete ${movieName} ?`, text: `You are about to remove ${movieName} from your rated movies`, buttons: ["Cancel", ` Delete ${movieName} `], dangerMode: true,
            })
                .then((willRemove) => {
                    if (willRemove) {
                        deleteMovie();
        ...
        return (
            <>
                <div className="col mt-5 uk-animation-fade-meduim" uk-scrollspy="cls: uk-animation-fade; target: .card; delay: 300; repeat: true">
                    <div className="card">
                        <Link to={`/${currentMovieName}?id=${currentMovieId}`}
                        ...
                        <div className="card-content">
                            ...
                            <div className="movie-content">
                                ...
                                <div className="movie-action-buttons">
                                    <div className="watchlist-btn peep-view" title="View Info" onClick={() => this.viewMore(movie)}>
...

                                    {
                                        isLoading ?
                                            ...
                                            <Link className="watchlist-btn" onClick={() => this.promptDelete(movie)}>
                                                <i className="fa fa-trash" title={`Remove ${currentMovieName} from rated movies`}></i>
                                            </Link>
                                    }
                                    <Link className="movie-title" to={`/${currentMovieName}?id=${currentMovieId}`}
                                        onClick={this.someMethod}>
