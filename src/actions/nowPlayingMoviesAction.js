...
export const fetchInitialNowPlayingMovies =
  () => async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch({
        type: IS_INITIAL_NOW_PLAYING_MOVIES_SUCCESS,
        payload: nowPlayingMovies.data.results,
      });
    } catch (error) {
      console.log(error);
...
export const fetchMoreNowPlayingMovies = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const updatedPopularMovies = [
      ...getState().nowPlayingMovies.movies,
      ...nowPlayingMovies.data.results,
    ];
    dispatch({
      type: FETCH_MORE_NOW_PLAYING_MOVIES_SUCCESS,
