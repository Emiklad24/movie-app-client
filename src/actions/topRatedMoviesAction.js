...
export const fetchInitialTopRatedMovies = () => async (dispatch, getState) => {
  ...
  try {
    let topRatedMovies = await axios.get(
    ...
    dispatch({
      type: IS_INITIAL_TOP_RATED_MOVIES_SUCCESS,
      payload: topRatedMovies.data.results,
    });
  } catch (error) {
    console.log(error);
...
export const fetchMoreTopRatedMovies = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const updatedTopRatedMovies = [
      ...getState().topRatedMovies.movies,
      ...topRatedMovies.data.results,
    ];
    dispatch({
      type: FETCH_MORE_TOP_RATED_MOVIES_SUCCESS,
      payload: updatedTopRatedMovies,
    });
  } catch (error) {
    console.log(error);
