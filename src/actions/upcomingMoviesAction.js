...
export const fetchInitialUpcomingMovies = () => async (dispatch, getState) => {
  ...
  try {
    let upcomingMovies = await axios.get(
    ...
    dispatch({
      type: IS_INITIAL_UPCOMING_MOVIES_SUCCESS,
      payload: upcomingMovies.data.results,
    });
  } catch (error) {
    console.log(error);
...
export const fetchMoreUpcomingMovies = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const updatedUpcomingMovies = [
      ...getState().upcomingMovies.movies,
      ...upcomingMovies.data.results,
    ];
    dispatch({
      type: FETCH_MORE_UPCOMING_MOVIES_SUCCESS,
      payload: updatedUpcomingMovies,
    });
  } catch (error) {
    console.log(error);
